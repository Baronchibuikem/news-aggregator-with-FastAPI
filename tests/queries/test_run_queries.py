...
from src.utils.external_libs_config import (EXTERNAL_SOURCES_CONFIG, API_COLLECTION, 
    NEWS_API_CONFIG, REDDIT_API_CONFIG)
...
def test_search_news_query_new_api_success():
    ...
    responses.add(
            responses.GET,
            EXTERNAL_SOURCES_CONFIG[NEWS_API_CONFIG]['search_url'].format(query=query_params, limit=2),
            json=expected_response,
            status=200,
            content_type='application/json',
            headers={'x-api-key': EXTERNAL_SOURCES_CONFIG[NEWS_API_CONFIG]['access_key']}
...
def test_search_news_query_new_api_failure():
    ...
    responses.add(
            responses.GET,
            UNREGISTERED_LIBRARY[TWITTER_API_CONFIG]['search_url'].format(query=query_params, limit=2),
            json=expected_response,
            status=200,
            content_type='application/json',
            headers={'x-api-key': EXTERNAL_SOURCES_CONFIG[NEWS_API_CONFIG]['access_key']}
...
def test_search_news_query_reddit_api_success():
    ...
    responses.add(
            responses.GET,
            EXTERNAL_SOURCES_CONFIG[REDDIT_API_CONFIG]['search_url'].format(query=query_params, limit=2),
            json=expected_response,
            status=200,
            content_type='application/json',
            headers={'User-agent': 'your bot 0.1'}
